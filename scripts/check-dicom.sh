...
# Usage: ./check-dicom.sh <dicom-file>

if [ $# -eq 0 ]; then
    echo "Usage: $0 <dicom-file>"
    echo "Example: $0 data/samples/MR000000.dcm"
    exit 1
fi
...
# Cek apakah file memiliki header DICOM yang valid (magic number "DICM" di byte 128)
echo -n "DICOM header: "
DICOM_MAGIC=$(dd if="$DICOM_FILE" bs=1 skip=128 count=4 2>/dev/null)
if [ "$DICOM_MAGIC" = "DICM" ]; then
    echo "✓ VALID"
else
    echo "✗ INVALID"
